...
def parse_mem_operand(lexer: lex.lexer):
    mem_operand = {}
    expect(lexer.curr_tok.type == "LBRACKET", SyntaxError, "Expected memory operand to start with Left Bracket")
    lexer.advance()
    expect(lexer.curr_tok.type == "REGISTER", SyntaxError, "Expected Register operand, instead found %s" % lexer.curr_tok.val)
    ...
    if lexer.curr_tok.type == "COMMA":
        lexer.advance()
        expect(lexer.curr_tok.type == "NUMBER", SyntaxError, "Expected Integer Literal operand, instead found %s" % lexer.curr_tok.val)
        mem_operand["offset"] = lexer.curr_tok.val
        lexer.advance()
    expect(lexer.curr_tok.type == "RBRACKET", SyntaxError, "Unexpected token %s, expected Right Bracket" % lexer.curr_tok.val)
    lexer.advance()
    if lexer.curr_tok.type == "COMMA":
        # post-index
        ...
        if lexer.curr_tok.type == "NUMBER":
            if mem_operand["offset"] != 0:
                raise SyntaxError("Cannot have both an immediate offset and a post-index offset")
            mem_operand["offset"] = lexer.curr_tok.val
            lexer.advance()
        else:
            raise SyntaxError("Expected Integer Literal operand, instead found %s" % lexer.curr_tok.val)
...
def parse_instr_statement(lexer: lex.lexer):
    ...
    # parse conditional jumps
        ...
        expect(lexer.curr_tok.type == "PERIOD", SyntaxError, "Expected period after J opcode")
        lexer.advance()
        expect(lexer.curr_tok.type == "STRING", SyntaxError, "Expected condition code following period in J.cc instruction")
        statement["condition_code"] = lexer.curr_tok.val
        lexer.advance()
        # Looking for one label
        lexer.advance()
        expect(lexer.curr_tok.type == "STRING", SyntaxError, "Expected a label after J.cc instruction, found %s-type instead" % lexer.curr_tok.type)
    ...
    # parse unconditional jumps, calls
        ...
        if lexer.curr_tok.type == "REGISTER":
            ...
            raise SyntaxError("Expected label or register for jump target, found %s-type instead" % lexer.curr_tok.type)
        lexer.advance()
    
    # parse loads and stores
    elif statement["opcode"] in ["LOADB", "STOREB", "LOADW", "STOREW"]:
        statement["instr_type"] = "load_store"
        expect(lexer.curr_tok.type == "REGISTER", SyntaxError, "Expected register token following opcode")
        statement["trf"] = lexer.curr_tok.val
        lexer.advance()
        expect(lexer.curr_tok.type == "COMMA", SyntaxError, "Expected comma following register token in instr %s" % statement)
        lexer.advance()
    ...
    # parse RR and RI instrs
        ...
        lexer.advance()
        expect(lexer.curr_tok.type == "COMMA", SyntaxError, "Expected comma following register token")
        lexer.advance()
        if lexer.curr_tok.type == "NUMBER":
            statement["instr_type"] = "ri_format"
            # RI-format
            expect(statement["opcode"] in parse_lookups.RI_FORMAT_INSTRS_LOOKUP, SyntaxError, "Numeric literal source not allowed for this instruction!")
            statement["immediate"] = lexer.curr_tok.val
            lexer.advance()
        else:
            statement["instr_type"] = "rr_format"
            expect(lexer.curr_tok.type == "REGISTER", SyntaxError, "Expected register token following comma")
            statement["src"] = lexer.curr_tok.val
            lexer.advance()
    
     # we should be looking at an EOL token right now
    if lexer.curr_tok.type != "EOL":
        raise SyntaxError("Unexpected token '%s' for instruction '%s'" % (lexer.curr_tok.val, statement["opcode"]))
    return statement
...
def parse_directive_statement(lexer: lex.lexer):
    ...
    lexer.advance()
    expect(lexer.curr_tok.type == "COLON", SyntaxError, "Label definitions must have a colon following them!")
    return statement
